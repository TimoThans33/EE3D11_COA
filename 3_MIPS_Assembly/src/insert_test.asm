		...
		.data
		testArray: .word 5, 4, 3, 2, 1 		#define an array			
		.text

		
		j 	insert	
		
insert:		la 	$a0, testArray 			# $t3 has the address of testArray
		addi 	$a1, $zero, 5			# let's say for now that length = 5
		addi 	$a2, $zero, 12			# value of a[i] from the main
		addi 	$a3, $zero, 2			# let's say that i = 2
							
							#

		addi	$s0, $a1, -1			# j = length - 1
for_insert:	blt 	$s0, $a3, exit_f_i		# if j<i go to exit
